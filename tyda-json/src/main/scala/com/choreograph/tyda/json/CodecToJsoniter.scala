...
object CodecToJsoniter {
  ...
  extension (in: JsonReader) {
    def tokenError(expected: Byte): Nothing = {
  ...
  def nestedOptionReader[T](element: Codec.Option[T]): Reader[Option[Option[T]]] = {
    given Codec[T] = element.element
    val structReader = reader[Option[(value: Option[T])]](summon)
    in => structReader(in).map(_.value)
  }

  def seqReader[T](elementCodec: Codec[T]): Reader[Seq[T]] = {
  ...
  def productReader[T](codec: Codec.Product[T]): Reader[T] = {
