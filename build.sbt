...
lazy val tydaFuzz = (project in file("tyda-fuzz"))
  .settings(name := "tyda-fuzz")
  .settings(commonSettings)
  .settings(Dependencies.tydaFuzz)
  .dependsOn(scalafixRules % ScalafixConfig)
  .dependsOn(tyda, tydaIterator, tydaRewrite)
  .enablePlugins(NoPublishPlugin)
...
lazy val tydaSpark3 = (project in file("tyda-spark"))
  ...
  .dependsOn(tydaFuzz % "test->compile")
