...
private class ExprOnSpark[T](cfs: Map[ExprNode.Reference[?], ColumnFactory[?]]) {
  ...
  def cfFromRef(ref: ExprNode.Reference[?]): ColumnFactory[?] =
  ...
  def convert(expr: ExprNode[?])(using spark: SparkSession): Column =
    expr match {
      ...
      case ExprNode.MakeSome(Nullable(e)) => struct(convert(e).as("value"))
      ...
      case ExprNode.MapGet(map, key) => element_at(convert(map), convert(key))
